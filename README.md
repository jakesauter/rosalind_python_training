...
I have decidied that it would be better to take some time to refresh on algorithmic concepts and have a more established introduction to how some algorithms are specifically applied to bioinformatics problems than to attempt to ad-hoc solutions that may not be optimal, now that the problems are begining to become a bit harder and less in the "learning how to use python effectivley" stage. I will be taking a break from posting to this repository for a short while, while I am reading Neil C. Jones' and Pavel A. Pevzner's **An Introduction to Bioinformatics Algorithms**. I look forward to taking on more of these Rosalind.info challenges once I have a more established background in Bioinformatics Algorithms. If I decide to complete any of the problems in this book for procatice I will be sure to post the solutions in this seciton of the repository.

[Meselson–Stahl experiment](https://en.wikipedia.org/wiki/Meselson%E2%80%93Stahl_experiment) -- Noted as the most beautiful experiment in biology, confirming the belief of Watson and Crick that DNA replication is semiconservative, that is that both double stranded DNA copies after replication each posses a single strand of DNA from the original strand and a newly synthesized strand.
